...
CREATE OR ALTER PROCEDURE addAngajatTransactional (@angajat_nume VARCHAR(30),@angajat_cnp VARCHAR(14),@angajat_codF INT)
AS
	SET NOCOUNT ON;
	BEGIN TRAN 
		BEGIN TRY
			...
			INSERT INTO LogTable VALUES('addTransactional','angajat',GETDATE())
			COMMIT TRAN
		END TRY 
	BEGIN CATCH 
		ROLLBACK TRAN
		print 'Transaction addAngajat rollbacked!'
	END CATCH

GO
CREATE OR ALTER PROCEDURE addMasinaTransactional (@masina_serie_sasiu VARCHAR(30),@masina_codMM INT)
AS
	SET NOCOUNT ON;
	BEGIN TRAN 
		BEGIN TRY
			...
			INSERT INTO LogTable VALUES('addTransactional','masina',GETDATE())
			COMMIT TRAN
		END TRY 
	BEGIN CATCH 
		ROLLBACK TRAN
		print 'Transaction addmasina rollbacked!'
	END CATCH

GO

CREATE OR ALTER PROCEDURE addAccesMasinaTransactional (@angajat_id INT, @masina_id INT)
AS
	SET NOCOUNT ON;
	BEGIN TRAN 
		BEGIN TRY
	...
	BEGIN CATCH 
		ROLLBACK TRAN
		print 'Transaction add acces_masina rollbacked!'
	END CATCH
...
AS 
	...
	EXEC addMasina "ZARB4WT8953230313",2  --  this is good
	EXEC addAccesMasina 1,1  --  this will fail
